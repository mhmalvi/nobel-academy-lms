        ...
        <div class="row">
            <div class="col-md-8">
                <div class="d-flex flex-column">
                    <img src="{{ asset('storage/courses/' . $course->thumbnail) }}" class="img-fluid" alt="Responsive image">
                </div>
                        ...
                        <li class="list-group-item bg-light">
                            <a href="" class="flex d-flex align-items-center text-body text-underline-0">
                                <span class="avatar mr-3">
                                    <img src="{{ asset('storage/users/' . $enrollment->teacher->user->photo) }}"
                                        alt="{{ $enrollment->teacher->first_name }}" class="avatar-img rounded-circle">
                                </span>
                                            ...
                                            @if (in_array($item->unit_code, $enrollment->core_units))
                                                <li class="list-group-item d-flex align-items-center active">
                                                    <div class="avatar avatar-sm mr-3">
                                                        <span
                                                            class="avatar-title rounded-circle bg-primary">{{ $i++ }}</span>
                                                    </div>
                                                    <div>
                                                        <a href="{{ route('core', $item->id) }}">{{ $item->unit_name }}</a>
                                                        <div class="text-muted"><span
                                                                class="material-icons icon-16pt icon-light">book</span>
                                                            {{ $item->unit_code }}</div>
                                                    </div>
                                                    <div class="ml-auto d-flex align-items-center">
                                                        <span class="badge badge-success mr-2"><i
                                                                class="material-icons icon-16pt">check_circle</i></span>
                                                    </div>
                                                </li>
                                            @endif
                                        @empty
                                    ...
                                    <ul class="list-group list-lessons">
                                        @forelse ($enrollment->course->units->where('unit_type', 'elective') as $item)
                                            @if (in_array($item->unit_code, unserialize($enrollment->elective_units)))
                                                <li class="list-group-item d-flex border-0">
                                                    <a href="{{ route('elective', $item->id) }}">{{ $item->unit_code }} -
                                                        {{ $item->unit_name }}</a>
                                                </li>
                                            @endif
                                        @empty
