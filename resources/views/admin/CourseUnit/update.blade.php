            ...
            <form action="{{route('admin.unit.update', $unit->id)}}" method="post" enctype="multipart/form-data">
                ...
                <div class="row form-group">
                    <div class="col-md-3">
                        ...
                        <input type="text" name="unit_code" class="form-control" placeholder="Short code of the unit..." value="{{$unit->unit_code}}"/>
                        @error('unit_code')
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="unit_type" class="form-control">
                            ...
                            <option value="core" {{(old('unit_type') == 'core' || $unit->unit_type == 'core') ? 'selected' : ''}}>Core Unit</option>
                            <option value="elective" {{(old('unit_type') == 'elective' || $unit->unit_type == 'elective') ? 'selected' : ''}}>Elective Unit</option>
                        </select>
                        @error('unit_type')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="course" class="form-control">
                            ...
                            @forelse ($courses as $item)
                                <option value="{{$item->id}}" {{(old('course') == $item->id || $unit->course_id == $item->id) ? 'selected' : ''}}>{{$item->course_name}}</option>
                            @empty
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="unit_name" class="form-control" placeholder="Title of the unit..." value="{{($unit->unit_name) ? $unit->unit_name : old('unit_name')}}"/>
                    @error('unit_name')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <textarea name="descriptions" class="form-control" id="des">{{($unit->descriptions) ? $unit->descriptions : old('descriptions')}}</textarea>
                    @error('descriptions')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
